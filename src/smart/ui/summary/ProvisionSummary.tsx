  ...
  return (
    <MGDDisplayPane> 
      {entries.length > 0 
        ? <>
            <ButtonGroup>
              <Button onClick={() => onSave(JSON.stringify(result), FILE_TYPE.JSON)}> Export to JSON </Button>
              <Button onClick={() => onSave(translateObjectToXML(result), FILE_TYPE.XML)}> Export to XML </Button>
              <Button onClick={() => onSave(resultToCSV(result), FILE_TYPE.CSV)}> Export to CSV </Button>
            </ButtonGroup>
            {entries.map(([doc, record]) => <DocumentSummary key={doc} doc={doc} record={record}/>)}
          </>
        : <Text>No result</Text>
      }
    </MGDDisplayPane>
...
const DocumentSummary: React.FC<{
  ...
  return (
    ...
    <fieldset>
      ...
      {Object.entries(record).sort(([c1], [c2]) => clauseSorter(c1, c2)).map(([clause, summary]) => 
        <ClauseSummary
          key={clause}
          clause={clause}
          summary={summary}
...
const ClauseSummary: React.FC<{
  ...
  return (
    <>
      <h3>{clause}{summary.title!=='' && ` ${summary.title}`}</h3>
      {provisions.length > 0 &&
        <ul>
          {provisions.map(p => <li key={p.id}>
...
function resultToCSV(result: Record<string, Record<string, ClauseSummary>>): string {
  let out = [
    'Document', 
    'Clause', 
    'Clause title',
