...
import { MeasureResult, MeasureRType, MTestItem, MTestReport } from '../../model/Measurement';
...
const MeasurementTooltip: React.FC<{
  ...
  const contents = report !== undefined
...
const DescribeReportList:React.FC<{ report: MTestReport }> = function ({report}) {
  return (
    <>
      {report.map((r, index) => <DescribeReport key={`report${index}`} report={r}/>) }
    </>
  )
}
...
const DescribeReport:React.FC<{ report: MTestItem }> = function ({ report }) {
  return (
    <>
      ...
      <DescriptionItem label='Evaluated value (left)' value={report.left.toString()} />
      <DescriptionItem label='Evaluated value (right)' value={report.right.toString()} />
