...
export function propagateReadings(readings: SensorReading[], source: EmissionSource[]): [ReadingRecord[], number] {
...
export function testMeasurement2060(model: EditorModel, records: ReadingRecord[]) {
  records.forEach((r, index) => {
    records[index] = {
      ...r,
      result: r.include.length > 0 ? checkModelMeasurement(model, getPAS2060Values(r)) : undefined,
      source: {
        ...r.source,
