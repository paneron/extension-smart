...
export class Approval extends GraphNode {  
      ...
      while (i < t.length) {
        ...
        if (i < t.length) {
          if (command == "modality") {
            ...
            console.error('Parsing error: approval. ID ' + id + ': Unknown keyword ' + command)
          }
        } else {
          console.error('Parsing error: approval. ID ' + id + ': Expecting value for ' + command)
        }
      }
    }
  }
  ...
  resolve(idreg:IDRegistry):void {
    ...
    if (this.approvertext != "") {
      let y = idreg.getObject(this.approvertext)
      if (y instanceof Role) {
        this.approver = <Role> y
      } else {
        console.error("Error in resolving IDs in approver for process " + this.id)
      }
    }
    for (let x of this.recordtext) {
      let y = idreg.getObject(x)
      if (y instanceof Registry) {
        this.records.push(y)
      } else {
        console.error("Error in resolving IDs in approval record for process " + this.id)
      }
    }        
