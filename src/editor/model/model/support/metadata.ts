...
export class Metadata {
  ...
  constructor(x:string) {
    if (x != "") {
      let t:Array<string> = tokenizer.tokenizePackage(x)
      let i:number  = 0    
      while (i < t.length) {
        ...
        if (i < t.length) {
          if (command == "title") {
            ...
            console.error('Parsing error: metadata. Unknown keyword ' + command)
          }
        } else {
          console.error('Parsing error: metadata. Expecting value for ' + command)
        }
      }
    }
  }
