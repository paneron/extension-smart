...
export class Subprocess {
      ...
      while (i < t.length) {
        ...
        if (i < t.length) {
          if (command == "elements") {
            ...
            console.error('Parsing error: subprocess. ID ' + id + ': Unknown keyword ' + command)
          }
        } else {
          console.error('Parsing error: subprocess. ID ' + id + ': Expecting value for ' + command)
        }
      }
    }
  }
  ...
  readElements(data: string) {
    ...
    while (i < t.length) {
      ...
      if (i < t.length) {
        let id = name.trim()
        let nc = new SubprocessComponent(id, t[i++])
        this.childs.push(nc)
        this.map.set(id, nc)        
      } else {
        console.error('Parsing error: elements in subprocess. Expecting value for ' + name)
      }
    }
  }
  ...
  readData(data: string) {
    ...
    while (i < t.length) {
      ...
      if (i < t.length) {
        ...
        console.error('Parsing error: data in subprocess. Expecting value for ' + name)
      }
    }
  }
  ...
  readEdges(data: string) {
    ...
    while (i < t.length) {      
      ...
      if (i < t.length) {
        ...
        console.error('Parsing error: edges in subprocess. Expecting value for ' + id)
      }
    }
  }
    ...
    while (i < t.length) {      
      ...
      if (i < t.length) {
        if (command == "x") {
          ...
          console.error('Parsing error: components in subprocess. Element ' + elm + ': Unknown keyword ' + command)
        }
      } else {
        console.error('Parsing error: edges in subprocess. Expecting value for ' + command)
      }
    }
  }

  resolve(idreg: IDRegistry) {
    let x = idreg.getObject(this.elmtext)
    if (x instanceof GraphNode) {
      this.element = x
    } else {
      console.error("Error in resolving IDs in from for graph node " + this.elmtext)
    }    
  }
