...
export class AIAgent {
  static xmlToModel(data:string):Model {
    let xml = XMLParser.parse(data)
    let front = xml.getChildByTagName("front")
    let body = xml.getChildByTagName("body")
    let m = new Model()
    initModel(m)
    ...
    if (body.length > 0 && m.root != null) {
      let secs = body[0].getChildByTagName("sec")
      secs.forEach((s) => {
...
function setMeta(m:Model, xml:XMLElement) {
  ...
  m.meta.edition = xml.getElementValueByPath(["iso-meta", "std-ident", "edition"])
  m.meta.namespace = xml.getElementValueByPath(["iso-meta", "std-ident", "originator"])+xml.getElementValueByPath(["iso-meta", "std-ident", "doc-number"])
...
function readDetails(sec:XMLElement, m: Model, page:Subprocess) {
  let secno = sec.getElementValue("label")
  let sectitle = sec.getElementValue("title")
  let idreg = m.idreg

  let p = new Process(findUniqueID("Clause"+secno, idreg), "")
  p.name = sectitle
  idreg.addID(p.id, p)
  m.hps.push(p)
  let nc = new SubprocessComponent(p.id, "")
  nc.element = p
  page.map.set(p.id, nc)
  page.childs.push(nc)
  let nedge = new Edge(idreg.findUniqueEdgeID("Edge"), "")
  ...
  let refadded = false
  let ref = new Reference(idreg.findUniqueRefID("Ref"), "")
  ref.document = m.meta.title
  ref.clause = secno

  let roles = new Map<string, number>()
...
function processList(c:XMLElement, m:Model, p:Process, ref:Reference, refadded:boolean, roles:Map<string, number>):boolean {
        ...
        gc.getChilds().forEach((ggc) => {
          if (ggc instanceof XMLElement) {
            if (ggc.tag == "label") {
              ...
              refadded = addProvisions(ggc.toString(), m, p, ref, refadded, roles)
...
function addProvisions(statement:string, m:Model, process:Process, ref:Reference, refAdded:boolean, roles:Map<string, number>):boolean {  
...
function addProvision(statement:string, m:Model, process:Process, ref:Reference, refAdded:boolean, roles:Map<string, number>):boolean {
  let idreg = m.idreg
  let s = statement.trim()
  if (s != "") {
    let pro = new Provision(idreg.findUniqueProvisionID("Provision"), "")
    pro.condition = s
