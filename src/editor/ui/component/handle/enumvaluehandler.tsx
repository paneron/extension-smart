...
export class EnumValueHandler implements IList, IAddItem, IUpdateItem {
  ...
  constructor(reg: IEnum, updateObj:IEnumValue|null, setAdd:(b:boolean)=>void, setUpdate:(b:boolean)=>void, setUpdateAttribute:(x:IEnumValue)=>void, forceUpdate:()=>void, data:IEnumValue, setAttribute:(x:IEnumValue)=>void) {    
  ...
  getItems = ():Array<EnumValueListItem> =>  {
    let out:Array<EnumValueListItem> = []
  ...
  addClicked = () => {
    ...
    for (let a of this.parent.values) {
