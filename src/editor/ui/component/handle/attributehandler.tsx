...
export class AttributeHandler implements IList, IAddItem, IUpdateItem {
  ...
  constructor(model:Model, reg: IAttributeContainer, updateObj:IAttribute|null, setAdd:(b:boolean)=>void, setUpdate:(b:boolean)=>void, setUpdateAttribute:(x:IAttribute)=>void, forceUpdate:()=>void, data:IAttribute, setAttribute:(x:IAttribute)=>void) {
    this.model = model
  ...
  getItems = ():Array<AttributeListItem> =>  {
    let out:Array<AttributeListItem> = []
  ...
  addItemClicked = () => {
    this.data = {id:"", definition: "", cardinality:"", modality: "", ref:[], type: ""}    
  ...
  updateItem = (ref: string) => {
    let x = parseInt(ref)
    if (!isNaN(x)) {      
      let r = this.parent.attributes[x]
      this.data = {...r}        
      this.setData({ ...this.data })
  ...
  private getFields = (): Array<JSX.Element> => {
    ...
    let opt:Array<string> = []
    this.model.refs.map((r) => {opt.push(r.id)})
    let types:Array<string> = [...DATATYPE]    
    ...
    elms.push(<MultiReferenceSelector
      ...
      remove={        
        (x: Array<string>) => {              
          x.map((r) => {
            let index = this.data.ref.indexOf(r)            
            if (index != -1) {
  ...
  addClicked = () => {
    ...
    for (let a of this.parent.attributes) {
