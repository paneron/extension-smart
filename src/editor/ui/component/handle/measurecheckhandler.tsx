...
export class MeasureHandler implements IList, IAddItem, IUpdateItem {
  ...
  constructor(process: IProcess, oldIndex:number|null, setAdd:(b:boolean)=>void, setUpdate:(b:boolean)=>void, setOldIndex:(x:number)=>void, forceUpdate:()=>void, data:string, setData:(x:string)=>void) {    
  ...
  getItems = ():Array<MeasurementListItem> =>  {
    let out:Array<MeasurementListItem> = []    
  ...
  private getFields = (): Array<JSX.Element> => {
    ...
    elms.push(<div key="ui#measurement#validatorbutton#holder">
      <button key="ui#measurement#validatorbutton" onClick={() => validCheck(this.data)}> Validator check </button>
    </div>) 
    return elms
  }
