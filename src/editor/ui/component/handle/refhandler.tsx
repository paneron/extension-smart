...
export class RefHandler implements IList, IAddItem, IUpdateItem {
  ...
  constructor(mw:ModelWrapper, updateObj:Reference|null, setAdd:(b:boolean)=>void, setUpdate:(b:boolean)=>void, setUpdateRef:(x:Reference)=>void, forceUpdate:()=>void, data:IRef, setRef:(x:IRef)=>void) {
    this.mw = mw
  ...
  removeItem = (refs:Array<string>) => {
    for (let i = refs.length-1;i>=0;i--) {
      let removed = this.mw.model.refs.splice(parseInt(refs[i]), 1)      
      if (removed.length > 0) {
        let r = removed[0]
        for (let p of this.mw.model.provisions) {
        ...
        for (let app of this.mw.model.aps) {
        ...
        for (let d of this.mw.model.dcs) {
  ...
  addClicked = () => {
    ...
    if (this.data.clause.indexOf(",") != 0) {
      let rs = this.data.clause.split(",")
      for (let r of rs) {
        r = r.trim()
        let id = (this.data.refid + r.replaceAll(".", "-")).trim()
        if (!idreg.refs.has(id)) {                
          let ref = new Reference(id, "")      
      ...
      if (idreg.refs.has(this.data.refid)) {
        ...
        let ref = new Reference(this.data.refid, "")      
