...
const RegistryEditPage: React.FC<{ modelWrapper: ModelWrapper, isVisible: boolean }> = ({ modelWrapper, isVisible }) => {
  ...
  const [data, setData] = useState<IRegistry>({regid:"", regtitle:"", attributes:[]})

  const forceUpdate = () => {
    setDummy(!dummy)
  }

  let handle = new RegistryHandler(modelWrapper, registry, setAddMode, setUpdateMode, setUpdateRegistry, forceUpdate, data, setData)
