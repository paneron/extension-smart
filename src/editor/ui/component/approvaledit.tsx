...
const EditApprovalPage: React.FC<StateMan> = (sm:StateMan) => {  
  ...
  let approval = sm.state.viewapproval
  ...
  const recordRemove = (x: Array<string>) => {    
    x.map((r) => {
      if (approval != null) {
        let index = approval.records.indexOf(r)            
        if (index != -1) {
  ...
  const refRemove = (x: Array<string>) => {    
    x.map((r) => {
      if (approval != null) {
        let index = approval.ref.indexOf(r)            
        if (index != -1) {
  ...
  if (approval != null) {
    elms.push(<NormalTextField key="field#approvalID" text="Approval ID" value={approval.id} update={setPID} />)
    elms.push(<NormalTextField key="field#approvalName" text="Approval Process Name" value={approval.name} update={setAPPName} />)
    elms.push(<NormalComboBox  key="field#approvalModality" text="Modality" value={approval.modality} options = {MODAILITYOPTIONS} update={setAPPModality} />)    
    elms.push(<ReferenceSelector key="field#ApprovalActor" text="Actor" filterName="Actor filter" value={approval.actor} options = {roles} update={setActor} />)        
    elms.push(<ReferenceSelector key="field#ApprovalApprover" text="Approver" filterName="Approver filter" value={approval.approver} options = {roles} update={setAppprover} />)
    ...
    elms.push(<MultiReferenceSelector 
      ...
      remove={refRemove} />)    
    return (
      <DisplayPane style={{display: sm.state.viewapproval!=null?"inline":"none"}}>
        <MyCloseButtons onClick={() => close()}>X</MyCloseButtons> 
        {elms}        
        <div>
          <button key="processedit#saveButton" onClick={() => save(sm, sm.state.approval, approval)}> Save </button>
...
function save(sm:StateMan, oldValue:Approval|null, newValue:IApproval|null) {
  if (oldValue != null && newValue != null) {
    ...
    if (newValue.approver == "") {
      ...
      let approver = idreg.getObject(newValue.approver)
