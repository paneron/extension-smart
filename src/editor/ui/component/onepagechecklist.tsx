...
const OnePageChecklist: React.FC<{sm:StateMan, cond:ISearch}> = ({sm, cond}) => {    
  const close = () => {
    sm.state.onepage = false
    sm.setState(sm.state)
  }

  let data = new Map<string, Map<string, Array<string>>>()
  ...
  mw.model.hps.map((p) => {
    if (cond.actor == "" || (p.actor != null && p.actor.name == cond.actor)) {
      p.provision.map((s) => {
        s.ref.map((r) => {
          if (cond.document == "" || (cond.document == r.document && (cond.clause == "" || cond.clause == r.clause))) {
            let doc = data.get(r.document)
            if (doc == undefined) {
              console.error("Reference document not found")
            } else {
              let clause = doc.get(r.clause)
              if (clause == undefined) {
    ...
    mw.model.regs.map((reg) => {
      if (reg.data != null) {
        reg.data.attributes.map((a) => {
          a.ref.map((ref) => {
            if (cond.document == "" || (cond.document == ref.document && (cond.clause == "" || cond.clause == ref.clause))) {
              let doc = data.get(ref.document)
              if (doc == undefined) {
                console.error("Reference document not found")
              } else {
                let clause = doc.get(ref.clause)
                if (clause == undefined) {              
                  console.error("Reference clause not found")
                } else {
  ...
  let docs:Array<CLGroup> = []
  data.forEach((d, did) => {
    let newgroup = new CLGroup(did)
    docs.push(newgroup)
    d.forEach((c, cid) => {
      if (c.length > 0) {
        let parts = cid.split(".")
...
function recursiveAdd(g:CLGroup, level:number, docindex:number, elms:Array<JSX.Element>) {
  if (g.gmember.length > 0) {
    g.gmember.forEach((c) => {
      if (level == 0) {
        elms.push(<h2 key={"ui#checklist#doc"+docindex+"clause"+c.id}>{c.id} </h2>)
      } else if (level == 1) {
        elms.push(<h3 key={"ui#checklist#doc"+docindex+"clause"+c.id}>{c.id} </h3>)
      } else {
        elms.push(<p key={"ui#checklist#doc"+docindex+"clause"+c.id}>{c.id} </p>)
      }
      recursiveAdd(c, level+1, docindex, elms)
    })
  }
  let opt:Array<JSX.Element> = []
  g.items.map((s, index) => {
    opt.push(<li key={"ui#checklist#doc"+docindex+"provision"+index}> {s} </li>)
  })
  elms.push(<ul key={"ui#checklist#doc"+docindex+"provisionlist"+g.id}>{opt}</ul>)        
}
