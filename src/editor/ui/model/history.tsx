...
export class PageHistory {
  ...
  getPath(sm:StateMan):JSX.Element {    
    const goToPage = (i: number) => {
      ...
      let page = sm.state.history.popUntil(i)                    
      sm.state.modelWrapper.page = page
      sm.setState(sm.state)      
    }
    
    let name = sm.state.modelWrapper.model.meta.namespace == ""?"root":sm.state.modelWrapper.model.meta.namespace
    let elms:JSX.Element[] = [<button key={"ui#rootpathbutton"} onClick={() => goToPage(-1)}>{name}</button>]
    for (let i = 0; i < this.history.length;i++) {
      elms.push(<span key={"ui#pathseparator"+i}> / </span>)      
      elms.push(<button key={"ui#pathbutton"+i} onClick={() => goToPage(i)}>{this.pathtext[i]}</button>)
    }
    return <>{elms}</>
  }
  ...
  getMapperPath(sm:ModelViewStateMan):JSX.Element {
    ...
    let his = sm.state.history    

    const goToPage = (i: number) => {
      MapperFunctions.saveLayout(sm)
      let page = his.popUntil(i)
      mw.page = page
      MapperFunctions.updateMapRef(sm)
      sm.setState(sm.state)      
    }
    
    let name = mw.model.meta.namespace == ""?"root":mw.model.meta.namespace
    let elms:JSX.Element[] = [<button key={"ui#rootpathbutton"} onClick={() => goToPage(-1)}>{name}</button>]
    for (let i = 0; i < this.history.length;i++) {
      elms.push(<span key={"ui#pathseparator"+i}> / </span>)      
      elms.push(<button key={"ui#pathbutton"+i} onClick={() => goToPage(i)}>{this.pathtext[i]}</button>)
  ...
  getRootPage(): Subprocess {
    ...
    if (this.modelType == null) {
      ...
      let sm  = MapperFunctions.getStateMan(this.modelType)
