...
const MapperPathPane: React.FC<ModelViewStateMan> = (sm:ModelViewStateMan) => {
  ...
  return (
  <>
    <PathLabel key="ui#pathlabel">{sm.state.type==ModelType.ImplementationModel?"Implementation model":"Reference model"} : {his.getMapperPath(sm)} </PathLabel>
    <DrillUpButton key="ui#drillupbutton" onClick={() => drillup()} >Drill up</DrillUpButton>
  </>
  )
}
