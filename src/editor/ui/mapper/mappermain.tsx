...
const ModelMapper: React.FC<{isVisible:boolean}> = ({isVisible}) => {
  let [mState, setMState] = useState<MapLinkState> ({
  ...
  if (isVisible) {
    ret = <div>
      <ContainerRight>
        <ModelView maps={mState.dest} isMap={mState.isMap} setIsMap={setIsMap} forceupdate={updateLoc} type={ModelType.ReferenceModel}/>
      </ContainerRight>
      <ContainerLeft>
        <ModelView maps={mState.source} isMap={mState.isMap} setIsMap={setIsMap} forceupdate={updateLoc} type={ModelType.ImplementationModel}/>
      </ContainerLeft>
