...
import ReactFlow, { Controls, OnLoadParams, ControlButton, ReactFlowProvider } from 'react-flow-renderer'
import { ModelWrapper } from '../model/modelwrapper'
import { PageHistory } from '../model/history'
import * as factory from '../../model/util/componentFactory'
import { IMapModelState, ModelType, ModelViewStateMan } from './model/mapperstate'
...
const ModelView: React.FC<{type:ModelType, maps:Map<string, MappingProfile>, isMap:boolean, setIsMap:(x:boolean)=>void, forceupdate:()=>void}> = ({type, maps, isMap, setIsMap, forceupdate}) => {
  ...
  const infoButton = (
    <ControlButton style={state.infovisible?csson:cssoff} onClick={() => {
  ...
  const controlPaneButton = (
    <ControlButton style={state.cpvisible?csson:cssoff} onClick={() => {
  ...
  const dataVisibleButton = (
    <ControlButton style={state.datavisible?csson:cssoff} onClick={() => {
  ...
  const mapVisibleButton = (
    <ControlButton style={isMap?csson:cssoff} onClick={() => {
      setIsMap(!isMap)
      updateState(state)
    }}>
      Map
  ...
  let elms = state.modelWrapper.getReactFlowElementsFrom(state.modelWrapper.page, state.datavisible, false, state.type)  
  ...
  return (      
    <ReactFlowProvider>
      ...
      <MapperPathPane {...sm} />
      <MapperInfoPane {...sm} />
      <MapperControlPane key={"ControlPane#"+sm.state.type} sm={sm} elms={maps}/>      
    </ReactFlowProvider>
  )
}
