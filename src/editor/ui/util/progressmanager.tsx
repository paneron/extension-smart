...
export const initProgressManager = () => {
  ...
  let mw = functionCollection.getStateMan().state.modelWrapper
  let model = mw.model
  if (model.root != null && model.root.start != null) {
    exploreNode(model.root.start, visited, null)
    for (let x of model.root.childs) {
      if (!visited.has(x)) {
...
export const recalculateProgress = (root:Subprocess, modelWrapper:ModelWrapper) => {  
  let model = modelWrapper.model
  let visited = new Set<Dataclass>()
...
function setDoneStatus(x:SubprocessComponent, visited:Set<SubprocessComponent>) {
  x.isDone = true
  visited.add(x)
...
function calNodeProgress (y:SubprocessComponent, visited:Set<SubprocessComponent>):boolean {  
  let result = true
  visited.add(y)
  let x = y.element
  if (x instanceof EGate) {
    result = false
    for (let c of y.child) {
      if (c.to != null && !visited.has(c.to)) {
        let r = calNodeProgress(c.to, visited)
        result ||= r
      }
    }
    x.met = result
...
export const MyDataCheckBox: React.FC<{data: Dataclass, checkUpdated:()=>void}> = ({data, checkUpdated}) => {    
  return <MyCheckBox key={data.id+"#CheckBoxContainer"}>
    <input type="checkbox" key={data.id+"#CheckBox"} checked={data.isChecked} onChange={() => {
      setDataChecked(data)
      checkUpdated()
    }} />
    </MyCheckBox>
...
export const MyProcessCheckBox: React.FC<{data: Process|Approval, checkUpdated:()=>void}> = ({data, checkUpdated}) => {  
  return <MyCheckBox key={data.id+"#CheckBoxContainer"}>
    <input type="checkbox" key={data.id+"#CheckBox"} checked={data.isChecked} onChange={() => {
...
function exploreNode(y:SubprocessComponent, visited:Set<SubprocessComponent>, parent:GraphNode|null):Array<GraphNode|Provision> {  
      ...
      if (x.job == null) {
        ...
        if (x.page != null) {
          if (x.page.start != null) {
            jobs = jobs.concat(exploreNode(x.page.start, new Set<SubprocessComponent>(), x))
          }
        }
        x.job = jobs      
      }
...
function recursiveCheck(x:SubprocessComponent, b:boolean, visited:Set<SubprocessComponent>) {
  visited.add(x)
  if (x.element != null && x.element instanceof Process) {
    enforceProcess(x.element, b)
    if (x.element.page != null) {
      let start = x.element.page.start
      if (start != null) {
