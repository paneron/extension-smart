...
const LegendPane: React.FC<StateMan> = (sm:StateMan) => {
  ...
  if (state.mtestResult != null) {
    let legs = [MeasureRType.OK, MeasureRType.CONTAINERROR, MeasureRType.ERRORSOURCE]
    let desc = ["Test passed", "Component failed", "Test failed"]
    for (let i = 0;i < legs.length;i++) {
      elms.push(<Legend key={"ui#legend#"+i} color={getMeasureResultColor(legs[i])} text={desc[i]} />)
    }
  } else if (state.fpvisible) {
    let legs = [FilterType.EXACT_MATCH, FilterType.SUBPROCESS_MATCH]
    let desc = ["Filter match", "Component match"]
    for (let i = 0;i < legs.length;i++) {
      elms.push(<Legend key={"ui#legend#"+i} color={getFilterColor(legs[i])} text={desc[i]} />)
    }
  }
  
  return (  
    <LegendLabel key="ui#legendlabel"> 
      {elms}
    </LegendLabel>      
  )
}
