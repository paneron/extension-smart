...
import { setAttributeChecked, setProvisionChecked, setProvisionProgress } from "./progressmanager";
...
function descStart(x:StartEvent, isCheckListMode:boolean):JSX.Element {
  ...
  if (!isCheckListMode) {    
    elms.push(<button key={"ui#button#startSimulation"+x.id} onClick={()=>Simulator.startSimulation()}> Simulation </button>)    
  }
  return (
    <>
    {elms}    
...
function descEnd(x:EndEvent, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#removeEnd#"+x.id} onClick={()=>Cleaner.removeEvent(x)}> Remove </button>)
  }
  return (
    <>
    {elms}
...
function descProcess(x:Process, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#viewprocess#"+x.id} onClick={()=>functionCollection.viewEditProcess(x)}> Edit </button>)
    elms.push(<button key={"ui#button#removeprocess#"+x.id} onClick={()=>Cleaner.removeProcess(x)}> Remove </button>)
  }
  elms.push(<p key={x.id + "#ProcessID"}> Process: {x.id} </p>)
  elms.push(<p key={x.id + "#ProcessName"}> Name: {x.name} </p>)
  if (x.actor != null) elms.push(<p key={x.id + "#ProcessActor"}> Actor: {x.actor.name} </p>)
  if (x.modality != "") elms.push(<p key={x.id + "#Modality"}> Modality: {x.modality} </p>)  
  if (x.provision.length > 0) {
    let ps:Array<JSX.Element> = []
    x.provision.map((a:Provision) => ps.push(<li key={x.id+"#Pro#"+a.id}> {describeProvision(a, isCheckListMode)} </li>))
    elms.push(<p key={x.id + "#Provisions"}>Provisions</p>)
    elms.push(<ul key={x.id + "#ProvisionList" }>{ps}</ul>)
  }
  if (x.measure.length > 0) {
    let ms:Array<JSX.Element> = []
    x.measure.map((m:string, index:number) => ms.push(<li key={x.id+"#Measure#"+index}> {m} </li>))
...
function descApproval(x:Approval, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#viewapproval#"+x.id} onClick={()=>functionCollection.viewEditApproval(x)}> Edit </button>)
    elms.push(<button key={"ui#button#removeapproval#"+x.id} onClick={()=>Cleaner.removeApproval(x)}> Remove </button>)
  }
  elms.push(<p key={x.id+"#approvalLabel"}> Approval: {x.id} </p>)
  elms.push(<p key={x.id+"#nameLabel"}> Name: {x.name} </p>)
  if (x.actor != null) elms.push(<p key={x.id+"#actorLabel"}> Actor: {x.actor.name} </p>)
  if (x.approver != null) elms.push(<p key={x.id+"#approverLabel"}> Approver: {x.approver.name} </p>)
  if (x.modality != "") elms.push(<p key={x.id+"#modalityLabel"}> Modality: {x.modality} </p>)
  if (x.records.length > 0) {
    let ps:Array<JSX.Element> = []
    x.records.map((a:Registry) => ps.push(<li key={x.id+"#approvalRecord#"+a.id}> {a.title} </li>))
...
function descEGate(x:EGate, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#viewEGate#"+x.id} onClick={()=>functionCollection.viewEGate(x)}> Edit </button>)
    elms.push(<button key={"ui#button#removeEGate#"+x.id} onClick={()=>Cleaner.removeGate(x)}> Remove </button>)
  }
  return <>
    {elms}
...
function descSignalCatch(x:SignalCatchEvent, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#viewSignalCatch#"+x.id} onClick={()=>functionCollection.viewSignalCatch(x)}> Edit </button>)
    elms.push(<button key={"ui#button#removeSignalCatch#"+x.id} onClick={()=>Cleaner.removeEvent(x)}> Remove </button>)
  }
  return <>
    {elms}
...
function descTimer(x:TimerEvent, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#viewTimer#"+x.id} onClick={()=>functionCollection.viewTimer(x)}> Edit </button>)
    elms.push(<button key={"ui#button#removeTimer#"+x.id} onClick={()=>Cleaner.removeEvent(x)}> Remove </button>)    
  }
...
function describeRegistry(x:Registry, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  if (!isCheckListMode) {
    elms.push(<button key={"ui#button#datarepo#"+x.id} onClick={()=>functionCollection.viewDataRepository(x)}> Data repository </button>)
  }
  return <>
    {elms}
...
function describeDC(x:Dataclass, isCheckListMode:boolean): JSX.Element {
  ...
  <ul key={x.id+"#attributeList"}>
    {x.attributes.map((a:DataAttribute) => <li key={a.id}> {describeAttribute(a, isCheckListMode)} </li>)}  
  </ul>
  </>
}  

function describeAttribute(x:DataAttribute, isCheckListMode:boolean): JSX.Element {
  let elms:Array<JSX.Element> = []
  ...
  if (isCheckListMode) {
    elms.push(<p key={x.id+"#attributeIDLabel"}> <input type="checkbox" key={x.mother[0].id+"#"+x.id+"#CheckBox"} checked={x.isChecked} onChange={() => {
      setAttributeChecked(x)
      functionCollection.checkUpdated()
      }}/> <span style={css}>Attribute ID: {x.id} </span></p>)  
  } else {
    elms.push(<p key={x.id+"#attributeIDLabel"}> <span style={css}>Attribute ID: {x.id} </span></p>)  
  }  
  if (x.type != null && x.type != "") elms.push(<p key={x.id + "#attributeTypeLabel"} style={css}> Type: {x.type} </p>)
  if (x.cardinality != null && x.cardinality != "") elms.push(<p key={x.id + "#attributeCardinalityLabel"} style={css}> Cardinality: {x.cardinality} </p>)
  if (x.modality != "") elms.push(<p key={x.id + "#attributeModalityLabel"} style={css}> Modality: {x.modality} </p>)
  if (x.definition != "") elms.push(<p key={x.id + "#attributeDefinitionLabel"} style={css}> Definition: {x.definition} </p>)
...
function describeProvision(x:Provision, isCheckListMode:boolean):JSX.Element {
  let elms:Array<JSX.Element> = []
  const css:CSSProperties = {}
  if (x.modality == "SHALL") {
    css.textDecorationLine = 'underline'
  ...
  if (isCheckListMode) {
    elms.push(<p key={"ui#provisionStatementLabel#"+x.id}>
    <input type="checkbox" id={x.id+"#CheckBox"} checked={x.isChecked} onChange={(e) => {
      setProvisionChecked(x)
      functionCollection.checkUpdated()
      }}/>
    Statement: <span style={css}> {x.condition}</span></p>)
  } else {
    elms.push(<p key={"ui#provisionStatementLabel#"+x.id}> Statement: <span style={css}> {x.condition}</span></p>)
  }
  if (x.modality != "") elms.push(<p key={"ui#provisionModalityLabel#"+x.id}>Modality: {x.modality}</p>)
  if (x.ref.length > 0) {
    let ps:Array<JSX.Element> = []
    x.ref.map((a:Reference) => ps.push(<li key={a.id+"#ref#"+x.id}>     
    {a.toSummary()} </li>))
    elms.push(<p key={"ui#reflabel#"+x.id}>Reference:</p>)
    elms.push(<ul key={x.id+"#referenceList"}>{ps}</ul>)
  }
  if (isCheckListMode) {
    elms.push(<p key={"x.id#ProgressLabel"}>Progress: <input type="number" min="0" max="100" value={x.progress} onChange={(e) => progressUpdate(e, x)}></input>% </p>)
  }
	return <>{elms}</>
}
