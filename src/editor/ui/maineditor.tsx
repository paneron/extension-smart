...
import ReactFlow, { Controls, OnLoadParams, ControlButton, ReactFlowProvider, Connection, Edge, isNode } from 'react-flow-renderer'
...
import { edgeTypes, ISearch, IState, nodeTypes, SearchMan, StateMan } from './interface/state'
...
import { initProgressManager, recalculateEdgeHighlight, recalculateProgress } from './util/progressmanager'
...
const ModelEditor: React.FC<{isVisible:boolean}> = ({isVisible}) => {
  ...
  const searchman:SearchMan = {searchState:searchState, setSearchState:updateSearchState}
  ...
  console.debug("Debug message: editor", state, searchState, state.modelWrapper.model.idreg)
  
  const checkUpdated = () => {
    let modelwrapper = state.modelWrapper
    let root = modelwrapper.model.root
    if (root != null) {
  ...
  const onDrop = (event:React.DragEvent<any>) => {
    ...
    if (canvusRef.current != null && state.rfInstance != null) {      
      ...
      factory.addComponent(type, state.modelWrapper.model, state.modelWrapper.page, pos)      
      updateState(state)
    }
  }
  ...
  const viewEditProcess = (p:Process) => {
    state.viewprocess = {
      ...
      output: p == null?[]:p.output.flatMap((r) => {return r.id}),
      input: p == null?[]:p.input.flatMap((r) => {return r.id}),
      provision: p == null?[]:p.provision.flatMap((r) => {
  ...
  const viewEditApproval = (p:Approval) => {
    state.viewapproval = {
      ...
      records: p == null?[]:p.records.flatMap((r) => {return r.id}),
      ref: p == null?[]:p.ref.flatMap((r) => {return r.id}),      
    }
    state.approval = p
    updateState(state)
  }
  ...
  const connectHandle = (x:Edge<any>| Connection) => {    
    if (!state.clvisible && x.source != null && x.target != null) {
      ...
      let page = mw.page
      let s = page.map.get(x.source)
      let t = page.map.get(x.target)
      if (s != null && t != null && s instanceof SubprocessComponent && t instanceof SubprocessComponent) {
        if (s.element instanceof Dataclass || s.element instanceof Registry || t.element instanceof Dataclass || t.element instanceof Registry) {
          return
        }        
        let newEdge = new MyEdge(idreg.findUniqueEdgeID("Edge"), "")
  ...
  const controlPaneButton = (
    <ControlButton style={state.cvisible?csson:cssoff} onClick={() => {
  ...
  const dataVisibleButton = (
    <ControlButton style={state.dvisible?csson:cssoff} onClick={() => {
  ...
  const newComponentButton = (
    <ControlButton style={state.nvisible?csson:cssoff} onClick={() => {
  ...
  const basicSettingButton = (
    <ControlButton style={state.svisible?csson:cssoff} onClick={() => {
  ...
  const edgeDeleteButton = (
    <ControlButton style={state.edgeDeleteVisible?csson:cssoff} onClick={() => {
  ...
  const checkListButton = (
    <ControlButton style={state.clvisible?csson:cssoff} onClick={() => {
  ...
  const measurePaneButton = (
    <ControlButton style={state.measureVisible?csson:cssoff} onClick={() => {
  ...
  const filterPaneButton = (
    <ControlButton style={state.fpvisible?csson:cssoff} onClick={() => {
  ...
  const importButton = (
    <ControlButton style={state.importvisible?csson:cssoff} onClick={() => {
  ...
  const aiButton = (
    <ControlButton style={state.aivisible?csson:cssoff} onClick={() => {
  ...
  if (state.clvisible) {
    if (state.modelWrapper.model.root != null) {
      recalculateEdgeHighlight(state.modelWrapper.page, state.modelWrapper.model)
    }
  }  
  ...
  if (state.onepage) {
    elms.push(<OnePageChecklist key="OnePageChecklistPage" sm={sm} cond={searchState}/>)
  }
  ...
  if (isVisible) {    
    ...
    <Container>
      <ReactFlowProvider>      
        <ReactFlow
          key="MMELModel"
          elements={state.modelWrapper.getReactFlowElementsFrom(state.modelWrapper.page, state.dvisible, state.clvisible)}
        ...
        >
          <Controls>
            {dataVisibleButton}
            {state.simulation==null&&!state.clvisible?controlPaneButton:""}
            {state.simulation==null&&state.clvisible?filterPaneButton:""}
            {state.simulation==null&&state.clvisible?measurePaneButton:""}
            {state.simulation==null?checkListButton:""}
            {state.simulation==null&&!state.clvisible?basicSettingButton:""}
            {state.simulation==null&&!state.clvisible?newComponentButton:""}
            {state.simulation==null&&!state.clvisible?edgeDeleteButton:""}
        ...
        {state.measureVisible?<MeasureCheckPane /> : <InfoPane clvisible={state.clvisible}/> }
